...
const PricingPresentational: React.FC<BlockComponentProps<any>> = ({ data }) => {
          ...
          <div className="space-y-4">
            {(data.plans || []).map((plan: any, i: number) => (
              <div key={i} className={`p-4 border rounded-lg grid md:grid-cols-3 items-center gap-4 ${plan.highlighted ? `border-2 ${data.highlightColor || 'border-blue-600'}` : 'border-slate-200'}`}>
                <div className="md:col-span-2"><h3 className="text-xl font-semibold mb-1">{plan.name}</h3><p className="text-sm text-slate-500">{plan.description}</p></div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {(data.plans || []).map((plan: any, i: number) => (
            <div key={i} className={`p-6 border rounded-lg text-left flex flex-col ${plan.highlighted ? `border-2 ${data.highlightColor || 'border-blue-600'}` : 'border-slate-200'}`}>
              ...
              <ul className="text-sm text-slate-600 space-y-2 my-6 flex-grow">{(plan.features || []).map((feat: any, idx: number) => <li key={idx} className="flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-green-500"><path d="M20 6 9 17l-5-5" /></svg><span>{feat}</span></li>)}</ul>
