    ...
    <div className={`${data.backgroundColor || 'bg-white'} py-20 px-4`}>
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 items-center gap-12">
        ...
        <div className="text-left">
          <span className={`text-sm font-bold uppercase tracking-widest ${data.tagColor || 'text-blue-600'}`}>{data.tag}</span>
          <h2 className={`font-bold my-4 text-4xl md:text-5xl ${data.titleColor || 'text-slate-800'}`}>{data.title}</h2>
          <p className={`mb-6 text-lg leading-relaxed ${data.descriptionColor || 'text-slate-600'}`}>{data.description}</p>
          <div className="flex items-center justify-between mb-8"><p className={`font-bold text-4xl ${data.priceColor || 'text-slate-900'}`}>{data.price}</p><div className="flex items-center gap-1">{stars}</div></div>
