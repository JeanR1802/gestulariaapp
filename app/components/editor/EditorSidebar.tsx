...
import { 
    ...
    Palette, Type, FileText, Home, Box, ShoppingBag, MessageSquare, Check,
    ShieldCheck, Zap, HelpCircle, Image as ImageIcon, MoreVertical
} from 'lucide-react';
...
// --- CONFIGURACIÓN DE CARPETAS MAESTRAS ---
const BLOCK_FOLDERS = [
    { 
        id: 'hero_group', 
        name: 'Portadas (Hero)', 
        icon: LayoutTemplate, 
        blockTypes: ['hero', 'hero_decision', 'hero_video', 'hero_split', 'hero_whatsapp', 'hero_countdown'] 
    },
    { 
        id: 'commerce_group', 
        name: 'Comercio', 
        icon: ShoppingBag, 
        blockTypes: ['catalog', 'pricing', 'featured_product'] 
    },
    { 
        id: 'trust_group', 
        name: 'Confianza', 
        icon: ShieldCheck, 
        blockTypes: ['testimonials', 'benefits', 'logos'] 
    },
    { 
        id: 'cta_group', 
        name: 'Conversión', 
        icon: Zap, 
        blockTypes: ['cta_banner', 'countdown', 'newsletter'] 
    },
    { 
        id: 'info_group', 
        name: 'Información', 
        icon: HelpCircle, 
        blockTypes: ['faq', 'steps', 'text_rich'] 
    },
    { 
        id: 'structure_group', 
        name: 'Estructura', 
        icon: Box, 
        blockTypes: ['header', 'footer'] 
    },
    { 
        id: 'media_group', 
        name: 'Multimedia', 
        icon: ImageIcon, 
        blockTypes: ['gallery', 'video_player', 'image'] 
    }
];

// MOCK PAGES (Si no vienen por props)
const DEFAULT_PAGES = [
...
export function EditorSidebar({ 
    ...
    const [selectedCategory, setSelectedCategory] = useState('Todos'); // Para templates
    ...
    // --- RENDERIZADO DEL EDITOR (MODO EDICIÓN) ---
            ...
            <div className="flex flex-col h-full bg-white animate-in slide-in-from-left-5 duration-300">
                ...
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-sm font-black text-slate-900">Editar {BlockConfig.name}</h3>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
                    {EditorComponent ? (
                        React.createElement(EditorComponent as React.ComponentType<any>, { data: editingBlock.data, updateData: (key: string, value: any) => onUpdateBlock && onUpdateBlock(editingBlock.id, key, value) })
                    ) : (
    ...
    return (
        <aside className={cn(
            "h-full z-30 flex bg-[#F9FAFB] border-r border-slate-200 transition-all duration-300 shadow-xl", 
            isOpen ? "translate-x-0" : "-translate-x-full absolute" 
        )}>
            
            {/* 1. BARRA DE ICONOS (Se oculta visualmente al editar para dar foco, o se deshabilita) */}
            <div className={cn(
            ...
            {/* 2. PANEL DESLIZANTE (320px o más si es editor) */}
            <div className={cn(
                ...
                {isEditingMode ? (
                    ...
                    // MODO NAVEGACIÓN (Menú Principal)
                    <>
                        {/* HEADER DEL MENÚ */}
                             ...
                             {activeTab === 'components' && activeFolderId ? (
                                ...
                                <div className="animate-in slide-in-from-left-4 duration-200">
                                    ...
                                    <p className="text-xs text-slate-500 mt-1 font-medium">
                                        {activeTab === 'components' ? 'Selecciona una categoría.' : 'Personaliza tu sitio.'}
                                    </p>
                                </div>
                             )}
                        </div>
                                            ...
                                            {BLOCK_FOLDERS.map((folder) => {
                                                ...
                                                return (
                                                    <button 
                                                        ...
                                                        className="flex flex-col items-center justify-center p-4 rounded-xl bg-white border border-slate-200 hover:border-blue-500 hover:shadow-lg hover:-translate-y-0.5 transition-all group aspect-[4/3]"
                                                    >
                                        ...
                                        <div className="grid grid-cols-1 gap-4 animate-in fade-in slide-in-from-right-4 duration-300">
                                            {currentFolderVariants.length > 0 ? currentFolderVariants.map((variant) => (
                                                <button 
                                                    ...
                                                    className="group relative w-full rounded-xl bg-white border border-slate-200 hover:border-blue-500 hover:ring-2 hover:ring-blue-500/20 hover:shadow-lg transition-all overflow-hidden text-left"
                                                >
                                                    ...
                                                    <div className="p-3 flex justify-between items-center">
                                                        <span className="font-bold text-xs text-slate-800 group-hover:text-blue-700">{variant.name}</span>
                                    ...
                                    <div className="space-y-2">
                                        {pages.map((page) => (
                                            <div 
                                                ...
                                                className={cn(
                                                    "group flex items-center justify-between p-3 rounded-xl border transition-all cursor-pointer bg-white hover:shadow-sm",
                                                    activePageId === page.id ? "border-blue-500 ring-1 ring-blue-500 shadow-md" : "border-slate-200 hover:border-blue-300"
                                                )}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center transition-colors", page.isHome ? "bg-blue-50 text-blue-600" : "bg-slate-50 text-slate-500")}>
                                                        {page.isHome ? <Home className="w-4 h-4" /> : <FileText className="w-4 h-4" />}
                                                    </div>
                                                    <div>
                                                        <p className={cn("text-xs font-bold", activePageId === page.id ? "text-slate-900" : "text-slate-700")}>{page.name}</p>
                                                        <p className="text-[10px] text-slate-400 font-mono">{page.slug}</p>
                                                    </div>
                                                </div>
                                                <button className="opacity-0 group-hover:opacity-100 p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-slate-700">
                                                    <MoreVertical className="w-3.5 h-3.5" />
                                                </button>
                                            </div>
                                        ))}
                                    </div>
                                    <button className="w-full py-2.5 border border-dashed border-slate-300 rounded-xl flex items-center justify-center gap-2 text-slate-500 font-bold text-xs hover:border-blue-400 hover:text-blue-600 hover:bg-white transition-all">
                                        <Plus className="w-3.5 h-3.5" /> Añadir Página
                                    </button>
                                </div>
                            )}
                            ...
                            {activeTab === 'templates' && (
                                <div className="space-y-4 animate-in fade-in">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input 
                                            type="text" placeholder="Buscar diseño..." 
                                            className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 outline-none"
                                            value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                                        />
                                    </div>
                                    <div className="grid gap-4">
                                        {filteredTemplates.map(([key, template]) => (
                                            <div key={key} onClick={() => onApplyTemplate(key)} className="group cursor-pointer bg-white rounded-xl overflow-hidden border border-slate-200 hover:border-blue-500 hover:shadow-lg transition-all relative">
                                                <div className="aspect-video bg-slate-50 border-b border-slate-100 group-hover:opacity-90 transition-opacity">
                                                    <SidebarTemplatePreview type={template.type} />
                                                </div>
                                                <div className="p-3 flex justify-between items-center">
                                                    <div>
                                                        <span className="font-bold text-xs text-slate-800 block">{template.name}</span>
                                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{template.category}</span>
                                                    </div>
                                                    <div className="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                            ...
                            {activeTab === 'styles' && (
                                <div className="space-y-6 animate-in fade-in">
                                    <div className="space-y-3">
                                        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2"><Palette className="w-3 h-3" /> Colores</h3>
                                        <div className="grid gap-2">
                                            {COLOR_PALETTES.map((palette) => (
                                                <button 
                                                    key={palette.id}
                                                    onClick={() => { setActivePalette(palette.id); if(onUpdateTheme) onUpdateTheme('color', palette.id); }}
                                                    className={cn("w-full p-2 rounded-lg border flex items-center gap-3 transition-all bg-white hover:border-blue-300", activePalette === palette.id ? "border-blue-500 ring-1 ring-blue-500" : "border-slate-200")}
                                                >
                                                    <div className="flex -space-x-1">
                                                        {palette.colors.map((c, i) => <div key={i} className="w-4 h-4 rounded-full border border-white shadow-sm" style={{ backgroundColor: c }}></div>)}
                                                    </div>
                                                    <span className="text-xs font-bold text-slate-700">{palette.name}</span>
                                                    {activePalette === palette.id && <Check className="w-3 h-3 text-blue-500 ml-auto" />}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="space-y-3">
                                        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2"><Type className="w-3 h-3" /> Tipografía</h3>
                                        <div className="grid gap-2">
                                            {FONT_PAIRS.map((font) => (
                                                <button 
                                                    key={font.id}
                                                    onClick={() => { setActiveFont(font.id); if(onUpdateTheme) onUpdateTheme('font', font.id); }}
                                                    className={cn("w-full p-3 rounded-lg border text-left transition-all bg-white hover:border-blue-300 flex justify-between items-center", activeFont === font.id ? "border-blue-500 ring-1 ring-blue-500" : "border-slate-200")}
                                                >
                                                    <div>
                                                        <p className="text-sm leading-none text-slate-900 mb-0.5" style={{ fontFamily: font.title === 'Inter' ? 'sans-serif' : 'serif' }}>Aa</p>
                                                        <p className="text-[10px] text-slate-500 font-bold">{font.name}</p>
                                                    </div>
                                                    {activeFont === font.id && <Check className="w-3 h-3 text-blue-500" />}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
