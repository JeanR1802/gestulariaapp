...
// --- Interfaces de Datos ---
export interface PricingPlan { name: string; price: string; frequency: string; features: string[]; buttonText: string; featured?: boolean; }
...
// --- Director ---
export function PricingBlock({ data, isEditing, onUpdate }: BlockComponentProps<PricingData>) {
  const props = { data, isEditing, onUpdate };
  switch (data.variant) {
    case 'list':
      return <PricingList {...props} />;
    case 'simple':
      return <PricingSimple {...props} />;
    default:
...
function updatePlanArray(plans: PricingPlan[] = [], index: number, field: keyof PricingPlan, value: string | boolean | string[]): PricingPlan[] {
  const next = [...plans];
  next[index] = { ...next[index], [field]: value } as PricingPlan;
  return next;
}
function updatePlanFeature(plans: PricingPlan[] = [], planIndex: number, featureIndex: number, newValue: string): PricingPlan[] {
  const next = [...plans];
  const plan = next[planIndex];
  const features = [...(plan.features || [])];
  features[featureIndex] = newValue;
  next[planIndex] = { ...plan, features } as PricingPlan;
  return next;
}

// --- Componentes Visuales ---
const PricingColumns = ({ data, isEditing, onUpdate }: BlockComponentProps<PricingData>) => {
  ...
  const handleUpdate = (key: keyof PricingData, value: string | PricingPlan[]) => { if (onUpdate) onUpdate(key as string, value); };
  const handlePlan = (idx: number, field: keyof PricingPlan, value: string | boolean | string[]) => handleUpdate('plans', updatePlanArray(data.plans, idx, field, value));
  const handlePlanFeature = (pIdx: number, fIdx: number, value: string) => handleUpdate('plans', updatePlanFeature(data.plans, pIdx, fIdx, value));
  
  return (
    <div className={cn('py-16 px-4', bgStyles.className)} style={bgStyles.style}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <Editable tagName="h2" value={data.title} onUpdate={(v) => handleUpdate('title', v)} isEditing={isEditing} className={cn('font-bold', { 'text-4xl': isDesktop, 'text-3xl': isTablet, 'text-2xl': isMobile }, titleStyles.className)} style={titleStyles.style} />
          <Editable tagName="p" value={data.subtitle} onUpdate={(v) => handleUpdate('subtitle', v)} isEditing={isEditing} className={cn('mt-4 mx-auto max-w-2xl', { 'text-lg': isDesktop, 'text-base': isTablet || isMobile }, subtitleStyles.className)} style={subtitleStyles.style} />
        </div>
          ...
          {(data.plans || []).map((plan, index) => {
            ...
            return (
              <div key={index} className={cn('rounded-2xl p-8 shadow-lg border flex flex-col', cardBg.className, { 'border-blue-500 ring-2 ring-blue-500': plan.featured, 'border-slate-200': !plan.featured })} style={cardBg.style}>
                <Editable tagName="h3" value={plan.name} onUpdate={(v) => handlePlan(index, 'name', v)} isEditing={isEditing} className={cn('text-lg font-semibold', nameStyles.className)} style={nameStyles.style} />
                <div className="mt-4 flex items-baseline">
                  <Editable tagName="span" value={plan.price} onUpdate={(v) => handlePlan(index, 'price', v)} isEditing={isEditing} className={cn('text-4xl font-bold tracking-tight', priceStyles.className)} style={priceStyles.style} />
                  <Editable tagName="span" value={plan.frequency} onUpdate={(v) => handlePlan(index, 'frequency', v)} isEditing={isEditing} className={cn('ml-1 text-sm font-semibold', frequencyStyles.className)} style={frequencyStyles.style} />
                </div>
                <ul className="mt-8 space-y-3 text-sm flex-grow">
                  {(plan.features || []).map((feature, fIndex) => (
                    <li key={fIndex} className={cn('flex items-center gap-3', featureStyles.className)} style={featureStyles.style}>
                      ...
                      <Editable tagName="span" value={feature} onUpdate={(v) => handlePlanFeature(index, fIndex, v)} isEditing={isEditing} />
                    </li>
                  ))}
                </ul>
                <Editable tagName="a" value={plan.buttonText} onUpdate={(v) => handlePlan(index, 'buttonText', v)} isEditing={isEditing} className={cn('mt-10 block w-full text-center rounded-md px-3 py-2 text-sm font-semibold', buttonStyles.className)} style={buttonStyles.style} />
              </div>
            );
          })}
        </div>
...
const PricingList = ({ data, isEditing, onUpdate }: BlockComponentProps<PricingData>) => {
  return <div>Lista de Precios</div>;
};
const PricingSimple = ({ data, isEditing, onUpdate }: BlockComponentProps<PricingData>) => {
  return <div>Precios Simples</div>;
};
...
export function PricingStyleEditor({ data, updateData }: { data: PricingData; updateData: (key: keyof PricingData, value: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div><ColorPalette label="Color de Fondo" selectedColor={isCustomBg ? data.backgroundColor : data.backgroundColor || ''} onChange={(color) => updateData('backgroundColor', color)} /></div>
      <div><TextColorPalette label="Color del Título" selectedColor={isCustomTitle ? data.titleColor : data.titleColor || ''} onChange={(color) => updateData('titleColor', color)} /></div>
      <div><TextColorPalette label="Color del Subtítulo" selectedColor={isCustomSubtitle ? data.subtitleColor : data.subtitleColor || ''} onChange={(color) => updateData('subtitleColor', color)} /></div>
      <div className="border-t pt-4 mt-4">
        <h4 className="font-medium text-sm text-slate-600 mb-3">Estilo de los Planes</h4>
        <div><ColorPalette label="Color de Tarjeta" selectedColor={isCustomCard ? data.cardColor : data.cardColor || ''} onChange={(color) => updateData('cardColor', color)} /></div>
        <div><ColorPalette label="Color de Tarjeta Destacada" selectedColor={isCustomFeaturedCard ? data.featuredCardColor : data.featuredCardColor || ''} onChange={(color) => updateData('featuredCardColor', color)} /></div>
        <div><TextColorPalette label="Color Nombre del Plan" selectedColor={isCustomPlanName ? data.planNameColor : data.planNameColor || ''} onChange={(color) => updateData('planNameColor', color)} /></div>
        <div><TextColorPalette label="Color del Precio" selectedColor={isCustomPrice ? data.priceColor : data.priceColor || ''} onChange={(color) => updateData('priceColor', color)} /></div>
        <div><TextColorPalette label="Color de Frecuencia" selectedColor={isCustomFrequency ? data.frequencyColor : data.frequencyColor || ''} onChange={(color) => updateData('frequencyColor', color)} /></div>
        <div><TextColorPalette label="Color de Características" selectedColor={isCustomFeatureText ? data.featureTextColor : data.featureTextColor || ''} onChange={(color) => updateData('featureTextColor', color)} /></div>
