...
export const BlockWrapper = ({ children, isEditing, onEdit, onDelete, onMoveUp, onMoveDown, blockId, onToggleMobileToolbar, isMobileToolbarVisible }: BlockWrapperProps) => {
  ...
  const handleAction = (e: MouseEvent, action?: () => void) => {
    e.stopPropagation();
    if (action) action();
    onToggleMobileToolbar(null); // Siempre cierra la toolbar móvil después de una acción
  };
  ...
  return (
    ...
    >
      {/* Barra de herramientas para Desktop (basada en hover) */}
      {showDesktopToolbar && (
      ...
      {/* Barra de herramientas para Móvil (basada en toque) */}
