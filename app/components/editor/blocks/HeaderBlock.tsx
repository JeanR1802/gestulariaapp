// Archivo: app/components/editor/blocks/HeaderBlock.tsx (CÓDIGO INTELIGENTE Y DEFINITIVO)
'use client';
import React, { useState } from 'react';
import { InputField } from './InputField';

// 1. La interfaz de datos que incluye la propiedad 'variant'
...
// 2. Este es el componente que usa el PREVISUALIZADOR. Ahora es un "director".
export function HeaderBlock({ data }: { data: HeaderData }) {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const toggleMenu = () => setIsMenuOpen(!isMenuOpen);
  const sharedProps = { data, isMenuOpen, toggleMenu };

  // LA LÓGICA CLAVE: Este switch revisa la propiedad 'variant' y renderiza el diseño correcto.
  switch (data.variant) {
    case 'centered':
      return <HeaderCentered {...sharedProps} />;
    case 'withButton':
      return <HeaderWithButton {...sharedProps} />;
    case 'default':
    default:
      return <HeaderDefault {...sharedProps} />;
  }
}

// --- Iconos para el menú (sin cambios) ---
...
// --- Componentes de diseño internos (cada uno es una variante) ---
...
// El editor de campos ahora muestra campos condicionales
