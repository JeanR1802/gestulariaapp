...
// 2. Componentes de renderizado (sin cambios)
export function HeroBlock({ data }: { data: HeroData }) { /* ... código interno sin cambios ... */ }
const HeroDefault = ({ data }: { data: HeroData }) => ( <div className={`${data.backgroundColor || 'bg-slate-100'} p-12 md:p-20 text-center`}><h1 className={`text-3xl md:text-4xl font-bold mb-4 ${data.titleColor || 'text-slate-800'}`}>{data.title}</h1><p className={`text-lg mb-8 max-w-2xl mx-auto ${data.subtitleColor || 'text-slate-600'}`}>{data.subtitle}</p><a href={data.buttonLink || '#'} className={`inline-block px-6 py-2.5 rounded-md text-base font-semibold transition-transform hover:scale-105 ${data.buttonBgColor || 'bg-blue-600'} ${data.buttonTextColor || 'text-white'}`}>{data.buttonText}</a></div> );
const HeroLeftImage = ({ data }: { data: HeroData }) => ( <div className={`${data.backgroundColor || 'bg-white'}`}><div className="max-w-5xl mx-auto grid md:grid-cols-2 items-center gap-8 p-8 md:p-12"><div className="text-center md:text-left"><h1 className={`text-3xl md:text-4xl font-bold mb-4 ${data.titleColor || 'text-slate-800'}`}>{data.title}</h1><p className={`text-lg mb-8 ${data.subtitleColor || 'text-slate-600'}`}>{data.subtitle}</p><a href={data.buttonLink || '#'} className={`inline-block px-6 py-2.5 rounded-md text-base font-semibold transition-transform hover:scale-105 ${data.buttonBgColor || 'bg-blue-600'} ${data.buttonTextColor || 'text-white'}`}>{data.buttonText}</a></div><div><img src={data.imageUrl || 'https://placehold.co/600x400/e2e8f0/64748b?text=Imagen'} alt={data.title} className="rounded-lg shadow-lg mx-auto" /></div></div></div> );
const HeroDarkMinimal = ({ data }: { data: HeroData }) => ( <div className={`${data.backgroundColor || 'bg-slate-900'} p-12 md:p-24 text-center`}><h1 className={`text-4xl md:text-5xl font-bold mb-8 ${data.titleColor || 'text-white'}`}>{data.title}</h1><a href={data.buttonLink || '#'} className={`inline-block px-6 py-2.5 rounded-md text-base font-semibold transition-transform hover:scale-105 ${data.buttonBgColor || 'bg-white'} ${data.buttonTextColor || 'text-slate-800'}`}>{data.buttonText}</a></div> );
