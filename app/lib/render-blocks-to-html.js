  ...
  return blocks.map(block => {
    ...
    switch (type) {
      ...
      case 'hero':
        ...
        switch (data.variant) {
          case 'leftImage': 
            return `<div class="${data.backgroundColor || 'bg-white'}"><div class="max-w-5xl mx-auto grid md:grid-cols-2 items-center gap-8 p-8 md:p-12"><div class="text-center md:text-left"><h1 class="text-3xl md:text-4xl font-bold mb-4 ${titleClass}">${data.title}</h1><p class="text-lg mb-8 ${subtitleClass}">${data.subtitle}</p><a href="${data.buttonLink || '#'}" class="inline-block bg-blue-600 text-white px-6 py-2.5 rounded-md text-base font-semibold hover:bg-blue-700">${data.buttonText}</a></div><div><img src="${data.imageUrl || 'https://placehold.co/600x400/e2e8f0/64748b?text=Imagen'}" alt="${data.title}" class="rounded-lg shadow-lg mx-auto" /></div></div></div>`;
          case 'darkMinimal': 
            return `<div class="${data.backgroundColor || 'bg-slate-900'} p-12 md:p-24 text-center"><h1 class="text-4xl md:text-5xl font-bold mb-8 ${data.titleColor || 'text-white'}">${data.title}</h1><a href="${data.buttonLink || '#'}" class="inline-block bg-white text-slate-800 px-8 py-3 rounded-md text-base font-semibold hover:bg-slate-200">${data.buttonText}</a></div>`;
          case 'default': 
          default: 
            return `<div class="${data.backgroundColor || 'bg-slate-100'} p-12 md:p-20 text-center"><h1 class="text-3xl md:text-4xl font-bold mb-4 ${titleClass}">${data.title}</h1><p class="text-lg max-w-2xl mx-auto mb-8 ${subtitleClass}">${data.subtitle}</p><a href="${data.buttonLink || '#'}" class="inline-block bg-blue-600 text-white px-6 py-2.5 rounded-md text-base font-semibold hover:bg-blue-700">${data.buttonText}</a></div>`;
        }
