...
function EditPanel({ block, onUpdate, onClose }: EditPanelProps) {
  ...
  const updateData = (key: string, value: unknown) => { onUpdate({ data: { ...(block.data as object), [key]: value } }); };
  ...
  return (<div className="fixed right-0 top-0 bottom-0 w-80 bg-white border-l shadow-lg z-50 overflow-y-auto"><div className="p-4"><div className="flex items-center justify-between mb-6"><h3 className="text-lg font-semibold">✏️ Editar {getBlockName(block.type)}</h3><button onClick={onClose} className="text-gray-500 hover:text-gray-700">✕</button></div><div className="space-y-4">{block.type === 'hero' && (<>{(()=>{const d=block.data as HeroData;return(<><div><label className="block text-sm font-medium text-gray-700 mb-1">Título Principal</label><input type="text" value={d.title} onChange={(e) => updateData('title', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Subtítulo</label><textarea value={d.subtitle} onChange={(e) => updateData('subtitle', e.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Texto del Botón</label><input type="text" value={d.buttonText} onChange={(e) => updateData('buttonText', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div></>);})()}</>)}{block.type === 'text' && (<>{(()=>{const d=block.data as TextData;return(<div><label className="block text-sm font-medium text-gray-700 mb-1">Contenido</label><textarea value={d.content} onChange={(e) => updateData('content', e.target.value)} rows={8} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>);})()}</>)}{block.type === 'image' && (<>{(()=>{const d=block.data as ImageData;return(<><div><label className="block text-sm font-medium text-gray-700 mb-1">URL de la Imagen</label><input type="url" value={d.imageUrl} onChange={(e) => updateData('imageUrl', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Descripción (Alt)</label><input type="text" value={d.alt} onChange={(e) => updateData('alt', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Pie de foto</label><input type="text" value={d.caption} onChange={(e) => updateData('caption', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div></>);})()}</>)}{block.type === 'cards' && (<>{(()=>{const d=block.data as CardsData;return(<><div><label className="block text-sm font-medium text-gray-700 mb-1">Título de la Sección</label><input type="text" value={d.title} onChange={(e) => updateData('title', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /></div>{d.cards.map((card, index) => (<div key={index} className="border p-3 rounded-lg"><h4 className="font-medium text-sm mb-2">Tarjeta {index + 1}</h4><div className="space-y-2"><input type="text" value={card.icon} onChange={(e) => updateCardData(index, 'icon', e.target.value)} placeholder="Icono (Emoji)" className="w-full px-2 py-1 border rounded" /><input type="text" value={card.title} onChange={(e) => updateCardData(index, 'title', e.target.value)} placeholder="Título" className="w-full px-2 py-1 border rounded" /><textarea value={card.description} onChange={(e) => updateCardData(index, 'description', e.target.value)} placeholder="Descripción" className="w-full px-2 py-1 border rounded" /></div></div>))}</>);})()}</>)}{block.type === 'contact' && (<>{(()=>{const d=block.data as ContactData;return(<><div><label>Título</label><input type="text" value={d.title} onChange={(e) => updateData('title', e.target.value)} /></div><div><label><input type="checkbox" checked={d.showPhone} onChange={(e) => updateData('showPhone', e.target.checked)} /> Mostrar teléfono</label>{d.showPhone && (<input type="text" value={d.phone} onChange={(e) => updateData('phone', e.target.value)} />)}</div><div><label><input type="checkbox" checked={d.showEmail} onChange={(e) => updateData('showEmail', e.target.checked)} /> Mostrar email</label>{d.showEmail && (<input type="email" value={d.email} onChange={(e) => updateData('email', e.target.value)} />)}</div><div><label><input type="checkbox" checked={d.showAddress} onChange={(e) => updateData('showAddress', e.target.checked)} /> Mostrar dirección</label>{d.showAddress && (<textarea value={d.address} onChange={(e) => updateData('address', e.target.value)} />)}</div></>);})()}</>)}</div></div></div>);
}