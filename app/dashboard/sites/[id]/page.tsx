...
                    <BlockRenderer key={block.id} block={block} isEditing={editingBlock === block.id} onEdit={() => setEditingBlock(block.id)} onUpdate={(updates) => updateBlock(block.id, updates)} onDelete={() => deleteBlock(block.id)} onMoveUp={index > 0 ? () => moveBlock(index, index - 1) : null} onMoveDown={index < blocks.length - 1 ? () => moveBlock(index, index + 1) : null} />
                  ))}
                </div>
              )}
            </div>
...
      case 'text': return (<div className="prose max-w-none"><p className="text-gray-700 leading-relaxed">{block.data.content}</p></div>)
...
      case 'cards': return (<div className="py-8"><h2 className="text-3xl font-bold text-center text-gray-900 mb-12">{block.data.title}</h2><div className="grid md:grid-cols-3 gap-8">{block.data.cards.map((card, index) => (<div key={index} className="text-center p-6 bg-white rounded-lg shadow-sm"><div className="text-4xl mb-4">{card.icon}</div><h3 className="text-xl font-semibold mb-2">{card.title}</h3><p className="text-gray-600">{card.description}</p></div>))}</div></div>)
...
      case 'cta': return (<div className={`${block.data.backgroundColor} text-white p-12 rounded-lg text-center`}><h2 className="text-3xl font-bold mb-4">{block.data.title}</h2><p className="text-xl mb-8 opacity-90">{block.data.subtitle}</p><button className="bg-white text-blue-600 px-8 py-3 rounded-lg text-lg font-semibold hover:bg-gray-100">{block.data.buttonText}</button></div>)
